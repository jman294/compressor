...
int main (int argc, char ** argv) {
  ...
  for (int i = 0; i < NUM_CONTEXTS; i++) {
    if (contextCount[i] != 0) {
      ...
      /*printf(""BYTE_TO_BINARY_PATTERN""BYTE_TO_BINARY_PATTERN" %d\n", BYTE_TO_BINARY(i>>8), BYTE_TO_BINARY(i), predictions[i]);*/
    }
  }
  char *form = replaceWord(modelHeaderTemplate, "$1", argv[1]);
  char *predictionString = "";
  for (int i = 0; i < NUM_CONTEXTS; i++) {
    strcat(predictionString, predictions[i]);
    if (i < NUM_CONTEXTS - 1) {
      strcat(predictionString, ", ");
    }
  }
  form = replaceWord(form, "$2", predictionString);
  printf("%s", form);
}
