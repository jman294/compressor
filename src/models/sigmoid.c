...
/* Takes a float from 0 to 1 and applies logistic function */
int specialSigmoid (float x) {
  float L = 1.0;
  float x0 = 0.5;
  float k = 17.0;
  return (int) (MODEL_LIMIT*(L/(1 + exp(-k * (x - x0)))));
}
...
int main (int argc, char ** argv) {
  ...
  for (int i = 0; i < NUM_CONTEXTS; i++) {
    if (contextCount[i] != 0) {
      predictions[i] = specialSigmoid(((float) oneCount[i]) / contextCount[i]);
      /*printf("%d %d\n", (int) (4095 * ((float) oneCount[i] / contextCount[i])), predictions[i]);*/
    }
